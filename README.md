...
``` julia
vtkfile = vtk_grid("my_vtk_file", x, y, z) # 3D
vtkfile = vtk_grid("my_vtk_file", x, y)    # 2D
vtkfile = vtk_grid("my_vtk_file", x)       # 1D
```
...
Alternatively, in the case of structured grids, the grid points can be defined from a single array `points`, of dimensions `[dim, Ni, Nj, Nk]` where `dim` is the dimension (1, 2 or 3) and `Ni`,`Nj`,`Nk` the number of points in each direction `x`,`y`,`z`:

``` julia
vtkfile = vtk_grid("my_vtk_file", points)
```
...
```
  ...
  - `cell_type` is of type `VTKCellType` which contains the name and an integer value that determines the type of the cell, as
    defined in the
    [VTK specification](http://www.vtk.org/VTK/img/file-formats.pdf) (see figures 2 and 3 in that document). For convenience, WriteVTK includes a `VTKCellTypes` module that contains these definitions. For instance, a triangle is associated to the value `cell_type = VTKCellTypes.VTK_TRIANGLE`.
...
Alternatively, the grid points can be defined from 1-D arrays with equal length `num_points`:
